...
function PANEL:Init()
    ...
    self:SetTitle("AESTHETIC - リサフランク420 / 現代のコンピュー")
 ...
 onmousedown="return false;" src='https://raw.githubusercontent.com/iamcal/emoji-data/master/img-]] .. short .. [[-64/1f642.png'></img>
			<img unselectable="on"
 onselectstart="return false;"
 onmousedown="return false;" src="https://raw.githubusercontent.com/iamcal/emoji-data/master/img-]] .. short .. [[-64/1f44d.png"></img>
			<img unselectable="on"
 onselectstart="return false;"
 onmousedown="return false;" src="https://raw.githubusercontent.com/iamcal/emoji-data/master/img-]] .. short .. [[-64/1f680.png"></img>
			<img unselectable="on"
 onselectstart="return false;"
 onmousedown="return false;" src="https://raw.githubusercontent.com/iamcal/emoji-data/master/img-]] .. short .. [[-64/1f4a9.png"></img>
		]])
        ...
        local button = vgui.Create("rbreslow_RadioButton", buttonContainer)
        button:CenterVertical()
    ...
    if not speak.prefs:IsEnforced("avatars_type", "tag_position", "timestamps_enabled", "timestamps_type", "emoji_pack", "avatars_enabled", "emoji_enabled", "show_typing") then
        if not speak.prefs:IsEnforced("avatars_type", "avatars_enabled") then
            --[[ Begin: Avatar Style ]]
            heading(i18n:Translate("AVATAR_STYLE"))
        end

        if not speak.prefs:IsEnforced("avatars_type") then
        ...
        if not speak.prefs:IsEnforced("avatars_enabled") then
        ...
        if not speak.prefs:IsEnforced("tag_position") then
            --[[ Begin: Tag Position ]]
        ...
        if not speak.prefs:IsEnforced("timestamps_enabled", "timestamps_type", "show_typing") then
            ...
            local check = checkRow(i18n:Translate("HOUR"))
            ...
            local check = checkRow(i18n:Translate("DISPLAY_MESSAGE_ABOVE_HEAD"))
            check:SetValue(speak.prefs:Get("show_typing"))
            check.OnChange = function(self, value)
                speak.prefs:SetBoolean("show_typing", value)
            end
        ...
        if not speak.prefs:IsEnforced("emoji_pack", "emoji_enabled") then
            --[[ Begin: Emoji Style ]]
            heading(i18n:Translate("EMOJI_STYLE"))
        end

        if not speak.prefs:IsEnforced("emoji_pack") then
            padding(messageDisplay)
            ...
            local group = {
                ...
                emojiRow(i18n:Translate("EMOJI_ONE"), "emojione")
            }
        ...
        if not speak.prefs:IsEnforced("emoji_enabled") then
            padding(messageDisplay)
    ...
    if not speak.prefs:IsEnforced("notification_sound", "notification_enabled", "notification_duration") then
        messageDisplay = vgui.Create("DScrollPanel", root)

        if not speak.prefs:IsEnforced("notification_sound") then
            ...
            for i=1,#group do
                ...
                group[i].OnChange = function(self, value)
                    if value then
                        ...
                        if speak.Notifications[i] then
                            surface.PlaySound("speak/" .. speak.Notifications[i] .. ".mp3")
                        end
                    end
                end
            end
        ...
        if not speak.prefs:IsEnforced("notification_duration") then
            heading("Duration")
            padding(messageDisplay)
        ...
        if not speak.prefs:IsEnforced("notification_enabled") then
    ...
    if not speak.prefs:IsEnforced("timestamps_color", "font_name", "font_size", "font_weight", "font_border_type", "font_border_blur", "font_border_opacity") then
        local settings = vgui.Create("DProperties", root)

        if not speak.prefs:IsEnforced("timestamps_color") then
        ...
        if not speak.prefs:IsEnforced("font_name") then
        ...
        if not speak.prefs:IsEnforced("font_size") then
        ...
        if not speak.prefs:IsEnforced("font_weight") then
        ...
        if not speak.prefs:IsEnforced("font_border_type") then
        ...
        if not speak.prefs:IsEnforced("font_border_blur") then
        ...
        if not speak.prefs:IsEnforced("font_border_opacity") then
    ...
    local about = vgui.Create("DHTML", root)
    about:SetHTML([[
		<style> body { opacity: 0.9; color: white; } </style>
		<h1>Speak</h1>
		<p>Copyright &copy; 2016 Rocky "rbreslow" Breslow &lt;breslowrocky@gmail.com&gt;</p>
		<p>Special thanks to Josh "Acecool" Moser and Jack Wilsdon for consulting with me while I developed this project. Thank you Cronos Dage and Donkie for localizing Speak. Also, I'd like to mention Zerf, BFG9000, my friend Kevin, aStonedPenguin, and Corvezeo for voulenteering some of their time to me for testing.</p>

		<h2>Open Source:</h2>

		<p>The following libraries were written for use in Speak and have been open sourced to help the community.</p>

		<ul>
			<li><a href="#" onclick="return speak.OpenUrl('https://github.com/rbreslow/preferences.lua');">preferences.lua</a></li>
			<li><a href="#" onclick="return speak.OpenUrl('https://github.com/rbreslow/i18n.lua');">i18n.lua</a></li>
		</ul>

		<h2>Third-Party Attributions (notices are included with software):</h2>

		<ul>
			<li><a href="#" onclick="return speak.OpenUrl('https://github.com/iamcal/js-emoji');">js-emoji</a></li>
			<li><a href="#" onclick="return speak.OpenUrl('https://github.com/bryanwoods/autolink-js');">autolink-js</a></li>
		</ul>
	]])
    about:SizeToContents()
    about:AddFunction("speak", "OpenUrl", function(str)
        gui.OpenURL(str)
        return false
    end)

    root:AddSheet(i18n:Translate("ABOUT"), about, "icon16/information.png", false, false, i18n:Translate("ABOUT_POPOVER"))
